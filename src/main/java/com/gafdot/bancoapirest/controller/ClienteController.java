...
public class ClienteController {
	...
	@PutMapping("/{id}")
	public ResponseEntity<ClienteDtoDetalhado> alterar(@PathVariable Long id, @RequestBody @Valid ClienteForm form) {
		Optional<Cliente> optional = clienteRepository.findById(id);
		if (optional.isEmpty()) {
			return ResponseEntity.notFound().build();
		}
		return ResponseEntity.ok(form.alterar(optional.get()));
	}
